...
namespace HippoExchange
{
    public class Program
    {
        public static void Main(string[] args)
        {
            ...
            app.MapPut("/exchanges/{id}/approval",
                async ([FromServices] FirestoreDb db, string id, [FromBody] UpdateExchangeApprovalDto dto) =>
            {
            ...
            .WithOpenApi(op =>
            {
                ...
                op.Description = "Creates a new maintenance document with itemID, and description. The server sets CreatedUtc.";
                return op;
            });


            app.MapPut("/maintenance/{id}/description", async (FirestoreDb db, string id, UpdateMaintenanceDescriptionDto dto) =>
            {
            ...
            // PUT /maintenance/{id}/frequency — update frequency by document id
            app.MapPut("/maintenance/{id}/frequency", async (FirestoreDb db, string id, UpdateMaintenanceFrequencyDto dto) =>
            {
                var docRef = db.Collection("maintenance").Document(id);
                var snap = await docRef.GetSnapshotAsync();
            ...
            .WithName("UpdateMaintenanceFrequency")
            .WithTags("Maintenance")
            .Produces<Maintenance>(StatusCodes.Status200OK)
            .Produces(StatusCodes.Status404NotFound)
            .ProducesProblem(StatusCodes.Status400BadRequest)
            .WithOpenApi(op =>
            {
                op.Summary = "Update a maintenance record's frequency";
                op.Description = "Updates the frequency field of an existing maintenance document by its Firestore document id (e.g., 'Weekly', 'Monthly', 'Every 90 Days').";
                op.OperationId = "Maintenance_UpdateFrequency";
                return op;
            });
            ...
            app.MapGet("/messages/threads", async (FirestoreDb db, string userId, string? filter) =>
            {
                ...
                var q = db.Collection("messageThreads")
                          .WhereArrayContains("participants", userId)
                          .OrderByDescending("updatedUtc")
                          .Limit(100);

                var snaps = await q.GetSnapshotAsync();
                var threads = snaps.Select(s => s.ConvertTo<MessageThread>()).ToList();
            ...
            .WithOpenApi(op =>
            {
                ...
                op.Description = "Returns up to 100 threads the user participates in. Optional filter: all | unread | starred.";
                return op;
            });
            ...
            .WithOpenApi(op =>
            {
                ...
                op.Description = "Creates a message thread by canonical participant set. If one exists, returns it instead.";
                return op;
            });
            ...
            .WithOpenApi(op =>
            {
                ...
                op.Description = "Sets lastReadBy[userId] to current time.";
                return op;
            });
            ...
            .WithOpenApi(op =>
            {
                ...
                op.Description = "Adds or removes the userId in starredBy.";
                return op;
            });
    ...
    public record UpdateMaintenanceFrequencyDto(string Frequency);
